...
def phone_numbers(f):
    ...
    regex_phone_number = "(\d{3}[-\.\s]??\d{3}[-\.\s]??\d{3}|\(\d{3}\)\s*\d{3}[-\.\s]??\d{3})"
    phone_numbers = []
    with open(f,'r') as f:
        f = f.read()
        get_phone_numbers = re.findall(regex_phone_number, f)
    ...
    with open('assets/phone_numbers.txt','w') as f:
        for phone_number in sorted(phone_numbers):
            f.write(f"\n{str(phone_number)}\n")  
    shutil.copy('assets/existing-contacts.txt', 'assets/phone_numbers.txt')
